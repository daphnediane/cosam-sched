...
sub reorganize_files {
    ...
    for my $item (@items) {
        ...
        if ($item->{current_subdir} eq $target_subdir) {
            print STDERR "Already in correct location: $item->{prefix}-$item->{num}.md\n";
            next;
        }
        ...
        if (-f $target_path) {
            print STDERR "WARNING: Target file already exists: $target_path\n";
        ...
        if (move($source_path, $target_path)) {
            ...
            print STDERR "ERROR: Failed to move $source_path to $target_path: $!\n";
        }
    }
}
