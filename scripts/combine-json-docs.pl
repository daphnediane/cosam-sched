...
my $relative_json_path = File::Spec->abs2rel($json_schedule_dir, $output_dir);
...
for my $version (sort keys %versions) {
    ...
    my ($structures_section) = $entry_content =~ /## Structures\s*\n(.+?)(?=\n##|\z)/s;
    ...
    print $out "This document is generated from the structured documentation in [$relative_json_path]($relative_json_path).\n\n";
    print $out "---\n\n";
    ...
    for my $structure (@structure_summaries) {
        ...
        my ($access) = $structure->{content} =~ /## Access\s*\n(.+?)(?=\n##|\n#|\z)/s;
        my ($status) = $structure->{content} =~ /## Status\s*\n(.+?)(?=\n##|\n#|\z)/s;
        ...
        my ($fields_section) = $structure->{content} =~ /## Fields\s*\n(.+?)(?=\n##|\n#|\z)/s;
        if ($fields_section) {
            ...
            my ($table_only) = $fields_section =~ /(\| Field[^\n]*\n(?:\|[-\s\|]+\n)?(?:\|.*\n)*)/s;
            if ($table_only) {
        ...
        print $out "*See full details in: [`$structure->{file}`]($relative_file)*\n\n";
    }
    
    # Add examples section if available in entry file
    my ($examples_section) = $entry_content =~ /## Example(.+?)(?=\n##|\z)/s;
    if ($examples_section) {
    ...
    my ($migration_section) = $entry_content =~ /## Migration Notes(.+?)(?=\n##|\z)/s;
    ...
    print $out "- [JSON Schedule Documentation]($relative_json_path/) - Complete structured documentation\n";
    ...
    for my $other_version (sort keys %versions) {
        ...
        print $out "- [$other_config->{title}]($other_config->{output_file}) - $other_config->{description}\n";
    }
    
    print $out "\n";
    print $out "*This document is automatically generated. Do not edit directly.*\n";
...
sub discover_versions {
    ...
    opendir my $dh, $json_schedule_dir or die "Can't open $json_schedule_dir: $!";
    my @files = readdir $dh;
    closedir $dh;
    ...
    for my $file (@files) {
        ...
        if ($file =~ /^v(\d+)(?:-.+)?\.md$/) {
            my $version_num = $1;
            my $version_key = $file =~ /^(v\d+(?:-.+))\.md$/ ? $1 : "v$version_num";
            ...
            my @structures;
            my ($structures_section) = $content =~ /## Structures\s*\n(.+?)(?=\n##|\z)/s;
            
            if ($structures_section) {
                # Extract structure references like "- [meta](meta-v5.md)"
                while ($structures_section =~ /\[([^\]]+)\]\(([^)]+\.md)\)/g) {
                    my $structure_name = $1;
                    my $structure_file = $2;
            ...
            $versions{$version_key} = {
                ...
                title => $title || "Schedule JSON Format $version_key",
                description => $description || "JSON format documentation for $version_key",
                structures => \@structures
            };
            
            print STDERR "Discovered version: $version_key -> $output_file with " . scalar(@structures) . " structures\n";
        }
    }
