...
- **`extras` renamed to `metadata`**: The `extras` field from v5 is renamed to `metadata` for consistency with other item types. The format is unchanged — an object with string keys and values that are either plain strings or `{"formula": "...", "value": "..."}` objects for formula cells.
...
The `metadata` field stores non-standard key-value pairs from extra spreadsheet columns or user-defined fields. Each entry can be either:

- Plain string value for literal cells
- `{"formula": "...", "value": "..."}` object for formula cells with both formula and computed result

This field is only present in the full format and is stripped in the display variant.
...
### Workshop Session with Metadata
...
- `hidePanelist` affects display output by suppressing all credits
- Workshop-specific fields are optional but important for event planning
- `metadata` replaces the v5 `extras` field (same format, renamed for consistency)
