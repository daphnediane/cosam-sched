...
### Presenter Tag-String Find-or-Create

`PresenterEntityType::find_or_create_tagged(storage, input) -> Result<PresenterId, LookupError>`
is the **implementation** in `entity/presenter.rs`.
`Schedule::find_or_create_tagged_presenter(input)` is a **thin delegate**.

Accepts:

- **Tagged**: `G:Alice`, `P:<Bob=Crew`, `I:Carol==Team` (tag prefix sets rank)
- **Untagged with syntax**: `Alice=Group`, `<Alice==Group`, `==GroupName`
  (processed through `process_tagged` with no rank, defaults to Panelist for
  new entries but does not upgrade existing)
- **Bare name**: `Alice` (find-or-create with Panelist rank, no rank upgrade)
- **UUID**: `presenter-<uuid>` or raw UUID string

Rank handling: `Some(rank)` from tag upgrades existing presenters when it has
higher priority; `None` (bare/untagged) never upgrades. See IDEA-043 for future
read-only lookup.

`PresenterEntityType::find_or_create_by_name(storage, name, Option<PresenterRank>)`
is a public helper. `Some(rank)` upgrades existing; `None` creates with
Panelist default but never upgrades.

`LookupError`: `Empty`, `UuidNotFound`, `InvalidUuid`, `NameNotFound`
(reserved for IDEA-043), `UnknownTag`, `OtherSentinel`.

### EntityResolver Trait

`EntityResolver` (in `entity/mod.rs`) encapsulates field-value-to-ID resolution:

- `resolve_string()` — string to single ID
- `resolve_next_field_value()` — single `FieldValue` to IDs + work items
- `resolve_field_value()` — `FieldValue` to exactly one ID
- `resolve_field_values()` — `FieldValue` to zero or more IDs

Default implementations are generated by the macro for entities with
`#[default_resolver]`. `PresenterEntityType` provides a custom implementation
that delegates to `find_or_create_tagged` for tagged auto-creation.
