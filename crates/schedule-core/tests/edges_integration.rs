...
use schedule_core::tables::presenter::{
    PresenterCommonData, PresenterId, PresenterInternalData, EDGE_GROUPS, EDGE_MEMBERS,
};
...
fn test_inclusive_groups_transitive_closure() {
    ...
    sched
        .edge_add(group_id, EDGE_GROUPS, std::iter::once(parent_group_id))
        .expect("edge type validation failed");

    // inclusive_edges should return both direct and transitive groups
    let result = sched.inclusive_edges(member_id, EDGE_GROUPS);
...
fn test_inclusive_members_transitive_closure() {
    ...
    sched
        .edge_add(sub_member_id, EDGE_GROUPS, std::iter::once(member1_id))
        .expect("edge type validation failed");

    // inclusive_edges should return member1, member2, and transitive sub_member
    let result = sched.inclusive_edges(group_id, EDGE_MEMBERS);
...
fn test_edge_add_multiple_targets_symmetry() {
    ...
    assert_eq!(
        sched.connected_field_nodes(group1_id, EDGE_MEMBERS),
        vec![member_id.into()]
    );
    assert_eq!(
        sched.connected_field_nodes(group2_id, EDGE_MEMBERS),
        vec![member_id.into()]
    );
    ...
    assert!(sched
        ...
        .is_empty());
    assert!(sched
        .connected_field_nodes(group1_id, EDGE_MEMBERS)
        .is_empty());
    assert!(sched
        .connected_field_nodes(group2_id, EDGE_MEMBERS)
        .is_empty());
}
...
fn test_edge_add_from_group_side_symmetry() {
    ...
    sched
        .edge_add(group_id, EDGE_MEMBERS, std::iter::once(member_id))
        .expect("edge type validation failed");
    ...
    sched.edge_remove(group_id, EDGE_MEMBERS, std::iter::once(member_id));
...
fn test_edge_add_multiple_members_from_group_side() {
    ...
    assert_eq!(
        sched.connected_field_nodes(member1_id, EDGE_GROUPS),
        vec![group_id.into()]
    );
    assert_eq!(
        sched.connected_field_nodes(member2_id, EDGE_GROUPS),
        vec![group_id.into()]
    );
    ...
    assert!(sched
        ...
        .is_empty());
    assert!(sched
        .connected_field_nodes(member1_id, EDGE_GROUPS)
        .is_empty());
    assert!(sched
        .connected_field_nodes(member2_id, EDGE_GROUPS)
        .is_empty());
}
