...
use super::panel::{ExtraFields, ExtraValue};
...
use super::xlsx_import::{
    PresenterColumn, PresenterHeader, canonical_header, parse_presenter_header, rank_for_prefix,
};
...
struct UpdateSession {
    ...
    all_presenters: Vec<String>,
    credited_set: HashSet<String>,
    metadata: IndexMap<String, ExtraValue>,
    change_state: ChangeState,
    source: Option<SourceInfo>,
}

/// Flatten the panel hierarchy into updateable sessions with change tracking.
/// Deleted sessions are included so their spreadsheet rows can be marked.
...
fn write_presenter_columns(
    ...
    let mut covered_by_named: HashSet<String> = HashSet::new();
    ...
    for pc in &header_maps.presenter_cols {
        match &pc.header {
            ...
            PresenterHeader::Other => {
                let rank = rank_for_prefix(pc.rank_prefix).unwrap_or("guest");
...
fn update_panel_types_sheet(
    ...
    // Pre-scan: collect metadata keys that need new columns.
    let mut new_keys: Vec<String> = Vec::new();
...
mod tests {
    ...
    #[test]
    fn test_decode_presenter_name_double_eq_group() {
        assert_eq!(
            decode_presenter_name_from_header("Alice==UNC Staff"),
            "Alice"
        );
    ...
    #[test]
    fn test_decode_presenter_name_always_grouped_no_group() {
        assert_eq!(decode_presenter_name_from_header("<Bob"), "Bob");
    }
