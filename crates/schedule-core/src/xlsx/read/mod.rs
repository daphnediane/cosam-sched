...
pub fn import_xlsx(path: &Path, options: &XlsxImportOptions) -> Result<ScheduleFile> {
    ...
    // Build the schedule shell first, then populate via EditContext (import
    // mode — no undo tracking).
    ...
    // Populate rooms, panel types, and presenters through the edit module
    {
        ...
        panel_types::read_panel_types_into(
            &book,
    ...
    schedule.imported_sheets = ImportedSheetPresence {
        has_room_map: !schedule.rooms.is_empty()
            && schedule.rooms.iter().any(|r| r.source.is_some()),
        has_panel_types: !schedule.panel_types.is_empty()
            && schedule.panel_types.values().any(|pt| pt.source.is_some()),
        has_presenters,
        has_schedule: true,
    };
