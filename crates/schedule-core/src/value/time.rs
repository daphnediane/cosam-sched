    ...
    fn from(tr: TimeRange) -> Self {
        match tr {
            ...
            TimeRange::ScheduledWithEnd {
                ...
                start_time: Some(format_storage(start_time)),
                end_time: Some(format_storage(start_time + end_time_minus_start_time)),
                ..Default::default()
            },
        }
    }
...
pub enum TimeRange {
    ...
    /// Fully scheduled: start + end are canonical; the delta is stored so the
    /// variant cannot represent an inverted range.
    ///
    /// Although the field is a `Duration`, consumers must treat `end_time`
    /// (computed as `start_time + end_time_minus_start_time`) as the
    /// canonical value — any mutation that updates `start_time` while keeping
    /// the same "end" semantics adjusts this duration to preserve the end,
    /// not the interval length.
    ScheduledWithEnd {
        start_time: NaiveDateTime,
        end_time_minus_start_time: Duration,
    },
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TimeRange::ScheduledWithEnd {
                ...
                let end_time = *start_time + *end_time_minus_start_time;
                write!(
                    f,
                    "{} to {}",
                    start_time.format("%Y-%m-%d %H:%M"),
...
impl TimeRange {
    ...
    pub fn start_time(&self) -> Option<NaiveDateTime> {
        match self {
            TimeRange::UnspecifiedWithStart(s)
            | TimeRange::ScheduledWithDuration { start_time: s, .. }
            | TimeRange::ScheduledWithEnd { start_time: s, .. } => Some(*s),
            _ => None,
        }
    }
    ...
    pub fn end_time(&self) -> Option<NaiveDateTime> {
        match self {
            ...
            TimeRange::ScheduledWithEnd {
                start_time,
                end_time_minus_start_time,
            } => Some(*start_time + *end_time_minus_start_time),
            TimeRange::ScheduledWithDuration {
                start_time,
                duration,
    ...
    pub fn duration(&self) -> Option<Duration> {
        match self {
            ...
            TimeRange::ScheduledWithEnd {
                end_time_minus_start_time,
                ..
            } => Some(*end_time_minus_start_time),
            _ => None,
        }
    }
...
impl TimeRange {
        ...
        *self = match self {
            ...
            TimeRange::UnspecifiedWithEnd(e) => {
                if start_time < *e {
                    TimeRange::ScheduledWithEnd {
                        start_time,
                        end_time_minus_start_time: *e - start_time,
                    }
                } else {
                    TimeRange::UnspecifiedWithStart(start_time)
                }
            ...
            TimeRange::ScheduledWithEnd {
                ...
                let end_time = *old_start + *end_time_minus_start_time;
                if start_time < end_time {
                    TimeRange::ScheduledWithEnd {
                        start_time,
                        end_time_minus_start_time: end_time - start_time,
                    }
                } else {
                    TimeRange::UnspecifiedWithStart(start_time)
                }
        ...
        *self = match self {
            ...
            TimeRange::UnspecifiedWithStart(s) => {
                if end_time > *s {
                    TimeRange::ScheduledWithEnd {
                        start_time: *s,
                        end_time_minus_start_time: end_time - *s,
                    }
                } else {
    ...
    /// Remove the start time, preserving any duration or end time in an
    /// `Unspecified*` variant.
    pub fn remove_start_time(&mut self) {
        *self = match self {
            ...
            TimeRange::ScheduledWithEnd {
                start_time,
                end_time_minus_start_time,
            } => TimeRange::UnspecifiedWithEnd(*start_time + *end_time_minus_start_time),
            _ => return,
        };
    }
...
mod tests {
    ...
    #[test]
    fn accessors_scheduled_with_duration() {
    ...
    #[test]
    fn accessors_scheduled_with_end() {
        let s = dt("2026-06-26T14:00:00");
    ...
    fn overlaps() {
        let make = |start: &str, end: &str| {
            ...
            TimeRange::ScheduledWithEnd {
                start_time: s,
                end_time_minus_start_time: dt(end) - s,
            }
        };
    ...
    #[test]
    fn deserialize_inverted_range_falls_back_to_unspecified_with_end() {
        // Inverted start/end in the serialized form is unrepresentable in
        // `ScheduledWithEnd`; the deserializer keeps the end-time and drops
        // the incompatible start.
        let json = r#"{"start_time":"2026-06-26T15:00:00","end_time":"2026-06-26T14:00:00"}"#;
        let tr: TimeRange = serde_json::from_str(json).unwrap();
        assert_eq!(tr, TimeRange::UnspecifiedWithEnd(dt("2026-06-26T14:00:00")));
    }
    ...
    fn add_start_to_unspecified_with_end_valid() {
        ...
        assert_eq!(
            ...
            TimeRange::ScheduledWithEnd {
                start_time: dt("2026-06-26T14:00:00"),
                end_time_minus_start_time: Duration::minutes(60),
            }
        );
    }
    ...
    #[test]
    fn serde_round_trip_scheduled_with_duration() {
        let tr = TimeRange::ScheduledWithDuration {
            start_time: dt("2026-06-26T14:00:00"),
            duration: Duration::minutes(60),
        };
    ...
    #[test]
    fn serde_round_trip_scheduled_with_end() {
        let tr = TimeRange::ScheduledWithEnd {
            start_time: dt("2026-06-26T14:00:00"),
            end_time_minus_start_time: Duration::minutes(60),
        };
    ...
    #[test]
    fn serde_round_trip_unspecified() {
    ...
    #[test]
    fn serde_round_trip_unspecified_with_duration() {
        let tr = TimeRange::UnspecifiedWithDuration(Duration::minutes(45));
        let json = serde_json::to_string(&tr).unwrap();
        let back: TimeRange = serde_json::from_str(&json).unwrap();
        assert_eq!(tr, back);
    }
    ...
    fn display_formats() {
        ...
        assert!(tr.to_string().contains("60 min"));
        assert_eq!(TimeRange::Unspecified.to_string(), "Unspecified");
    }
}
