...
fn run(args: &[&str]) -> std::process::Output {
    Command::new(binary()).args(args).output().expect("failed to run cosam-modify")
}
...
fn test_help_exits_zero_and_prints_usage() {
    ...
    let text = stdout(&out);
    assert!(text.contains("USAGE"), "expected USAGE in help output:\n{text}");
    assert!(text.contains("--file"), "expected --file in help output:\n{text}");
}
...
fn test_missing_file_arg_exits_one() {
    ...
    assert_eq!(out.status.code(), Some(1), "expected exit 1 when --file is missing");
}
...
fn test_panel_type_create_list() {
    let f = tmp(".cosam");
    let create = run(&[
        "--file", f.to_str().unwrap(), "--new",
        "--select", "panel_type", "create", "prefix=GP", "kind=Guest Panel",
    ]);
    assert_eq!(create.status.code(), Some(0), "create failed:\n{}", stdout(&create));

    let list = run(&["--file", f.to_str().unwrap(), "--select", "panel_type", "list"]);
    assert_eq!(list.status.code(), Some(0));
    let out = stdout(&list);
    assert!(out.contains("GP"), "expected 'GP' in list:\n{out}");
    assert!(out.contains("Guest Panel"), "expected 'Guest Panel' in list:\n{out}");
...
fn test_presenter_create_list() {
    ...
    let create = run(&[
        "--file", f.to_str().unwrap(), "--new",
        "--select", "presenter", "create", "name=Alice",
    ]);
    assert_eq!(create.status.code(), Some(0));

    let list = run(&["--file", f.to_str().unwrap(), "--select", "presenter", "list"]);
    assert_eq!(list.status.code(), Some(0));
    let out = stdout(&list);
...
fn test_event_room_create_list() {
    ...
    let create = run(&[
        "--file", f.to_str().unwrap(), "--new",
        "--select", "event_room", "create", "name=Ballroom",
    ]);
    assert_eq!(create.status.code(), Some(0));

    let list = run(&["--file", f.to_str().unwrap(), "--select", "event_room", "list"]);
    assert_eq!(list.status.code(), Some(0));
...
fn test_hotel_room_create_list() {
    ...
    let create = run(&[
        "--file", f.to_str().unwrap(), "--new",
        "--select", "hotel_room", "create", "name=Room 101",
    ]);
    assert_eq!(create.status.code(), Some(0));

    let list = run(&["--file", f.to_str().unwrap(), "--select", "hotel_room", "list"]);
    assert_eq!(list.status.code(), Some(0));
...
fn test_panel_create_list() {
    ...
    let create = run(&[
        "--file", f.to_str().unwrap(), "--new",
        "--select", "panel_type", "create", "prefix=GP", "kind=Guest",
        "--", "--select", "panel", "create", "code=GP001", "name=Test Panel",
    ]);
    assert_eq!(create.status.code(), Some(0), "create failed:\n{}", stdout(&create));
    ...
    assert!(out.contains("Test Panel"), "expected 'Test Panel' in list:\n{out}");
    let _ = std::fs::remove_file(&f);
}
...
fn test_set_get_field() {
    ...
    let create = run(&[
        "--file", f.to_str().unwrap(), "--new",
        "--select", "panel_type", "create", "prefix=GP", "kind=Initial",
    ]);
    assert_eq!(create.status.code(), Some(0));

    let set = run(&[
        "--file", f.to_str().unwrap(),
        "--select", "panel_type", "GP", "set", "kind", "Updated Kind",
    ]);
    assert_eq!(set.status.code(), Some(0), "set failed:\n{}", stdout(&set));

    let get = run(&[
        "--file", f.to_str().unwrap(),
        "--select", "panel_type", "get", "GP",
    ]);
    assert_eq!(get.status.code(), Some(0));
    let out = stdout(&get);
    assert!(out.contains("Updated Kind"), "expected updated value in get:\n{out}");
    let _ = std::fs::remove_file(&f);
}
...
fn test_delete() {
    ...
    let setup = run(&[
        "--file", f.to_str().unwrap(), "--new",
        "--select", "panel_type", "create", "prefix=GP", "kind=Guest",
        "--", "--select", "panel_type", "create", "prefix=WP", "kind=Workshop",
    ]);
    assert_eq!(setup.status.code(), Some(0));

    let del = run(&[
        "--file", f.to_str().unwrap(),
        "--select", "panel_type", "delete", "GP",
    ]);
    assert_eq!(del.status.code(), Some(0), "delete failed:\n{}", stdout(&del));

    let list = run(&["--file", f.to_str().unwrap(), "--select", "panel_type", "list"]);
    let out = stdout(&list);
    assert!(!out.contains("GP"), "GP should be gone after delete:\n{out}");
...
fn test_add_remove_edge() {
    ...
    let setup = run(&[
        ...
        "--select", "panel_type", "create", "prefix=GP", "kind=Guest",
        "--", "--select", "presenter", "create", "name=Alice",
        "--", "--select", "panel", "create", "code=GP001", "name=EdgeTest",
        "--", "--select", "panel", "GP001", "add-edge", "credited_presenters", "Alice",
    ]);
    ...
    assert!(has_value, "credited_presenters should have a UUID value:\n{out}");

    // Remove the edge.
    let remove = run(&[
        "--file", f.to_str().unwrap(),
        "--select", "panel", "GP001", "remove-edge", "credited_presenters", "Alice",
    ]);
    assert_eq!(remove.status.code(), Some(0), "remove-edge failed:\n{}", stdout(&remove));

    // After removal, get should show no credited_presenters value.
    let get2 = run(&["--file", f.to_str().unwrap(), "--select", "panel", "get", "GP001"]);
    let out2 = stdout(&get2);
    let still_has_value = out2
        .lines()
        .any(|l| l.trim_start().starts_with("credited_presenters:") && l.contains('-'));
    assert!(!still_has_value, "credited_presenters should be empty after remove:\n{out2}");
...
fn test_undo_reverts_create() {
    ...
    let out = run(&[
        ...
        "--select", "panel_type", "create", "prefix=GP", "kind=Guest",
        "--", "undo",
        "--", "--select", "panel_type", "list",
    ]);
    assert_eq!(out.status.code(), Some(0), "undo run failed:\n{}", stdout(&out));
    let text = stdout(&out);
    assert!(text.trim().is_empty(), "list should be empty after undo:\n{text}");
    let _ = std::fs::remove_file(&f);
}
...
fn test_redo_reapplies_after_undo() {
    ...
    assert_eq!(out.status.code(), Some(0), "redo run failed:\n{}", stdout(&out));
    let text = stdout(&out);
...
fn test_undo_empty_stack_exits_one() {
    ...
    assert_eq!(out.status.code(), Some(1), "expected exit 1 for undo on empty stack");
    let _ = std::fs::remove_file(&f);
}
...
fn test_xlsx_to_binary_conversion() {
    ...
    let out = Command::new(binary())
        .args(["--file", xlsx.to_str().unwrap(), "--select", "panel", "list"])
        .output()
        .unwrap();
    assert_eq!(out.status.code(), Some(0), "xlsx list failed:\n{}", stdout(&out));
    let panels = stdout(&out);
    assert!(!panels.trim().is_empty(), "expected panels from xlsx:\n{panels}");
}
