...
use gpui::prelude::*;
use gpui::{
    anchored, deferred, div, px, rgb, Context, Menu, OwnedMenu, OwnedMenuItem, SharedString, Window,
};
...
pub(super) fn menus() -> Vec<Menu> {
    vec![
        ...
        Menu {
            name: "View".into(),
            items: super::view_menu_items(),
        },
        Menu {
            name: "Help".into(),
            items: super::help_menu_items(),
        },
    ]
}
            ...
            match item {
                ...
                OwnedMenuItem::Submenu(submenu) => {
                    ...
                    let row = div()
                        ...
                        .cursor_pointer()
                        .hover(|s| s.bg(rgb(0xE5E7EB)))
                        .child(SharedString::from(submenu_name))
                        .child(div().text_color(rgb(0x9CA3AF)).child("▶"));

                    popup =
                        popup.child(row.on_click(cx.listener(move |this, _event, _window, cx| {
...
impl Render for WindowsMenuBar {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if let (Some(menu_idx), Some(_sub_idx)) = (self.open_menu_index, self.open_submenu_index) {
            if let Some(menu) = self.menus.get(menu_idx) {
                for item in &menu.items {
                    if let OwnedMenuItem::Submenu(sub) = item {
                        let popup = self.render_submenu_popup(sub, window, cx);
                        root = root.child(deferred(anchored().child(popup)).with_priority(2));
                        break;
                    }
                }
            }
